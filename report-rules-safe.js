...
function addVariableChips(){
  ...
  help.innerHTML='<b>变量解析说明</b><br>1. 团抽：{团抽金额}{25%} → 订单总价 × 25% 算团抽金额<br>2. 团抽：{团抽比例}{25%} → 直接显示 25%<br>3. 派抽：{派抽金额}{10%} → 订单总价 × 10% 算派抽金额<br>4. 到手：{到手比例}{70%} → 直接显示 70%<br>5. 派单金额：{派单金额}{25%} → 订单总价 × 25% 计算<br><br>单独放 {25%} 也会在复制报备时按对应订单总价 × 25% 自动计算。旧模板里的 {团抽} / {派抽} / {到手} 继续兼容。';
...
function reportVars(){
  ...
  const staged=Array.isArray(window.paiMiniMultiOrder?.lines)&&window.paiMiniMultiOrder.lines.length?window.paiMiniMultiOrder.lines[window.paiMiniMultiOrder.lines.length-1]:null;
  const itemName=$('calcItemName')?.value.trim()||item.name||staged?.item||'';const unit=$('calcUnitLabel')?.value.trim()||item.unit_label||staged?.unit||'次';const unitPrice=num($('calcUnitPrice')?.value)||num(calc.unitPrice)||num(staged?.price);const measure=$('durationInput')?.value.trim()||calc.measure||staged?.measure||'';const quantity=calc.quantity??staged?.quantity??'';const original=num(calc.originalTotal)||total;const discount=num(calc.discountRate)||num(staged?.discountRate)||100;
  return {
