...
- 新增 `20260923033000_shop_membership_invites.sql`：加入 `shop_members` / `shop_invites`、店铺邀请码生成/关闭/重新生成、通过邀请码加入、退出店铺，以及新的店铺/价格表 RLS；旧 `visibility` 字段仅保留兼容，不再作为普通用户可见性依据。
- 新增 `shop-membership.js`：在「小店」同一区域提供输入邀请码加入、我加入的店铺、店主复制邀请码、复制邀请链接、关闭/开启邀请、重新生成邀请码等功能；邀请链接会自动把加入码带到小店页面。
- 店铺成员仅共享店铺资料和价格表；老板档案、消费记录、预存余额、权益、报备等仍按各自 `user_id` 隔离，不会因为加入同一店铺而互相可见。
- 管理端继续使用 `SECURITY DEFINER` 管理 RPC，因此店铺成员制不会限制管理端查看全部店铺；`admin_list_shops()` 同时增加成员数和邀请状态字段。
- 到期只读模式补充店铺成员操作保护：到期账号不能加入/退出店铺、关闭/重新生成店铺邀请，但仍可查看已有可见数据。
